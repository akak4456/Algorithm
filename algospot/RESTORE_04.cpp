	...
	*/
	...
	for (int nxt = 0;nxt < k;nxt++) {
		if ((used & (1 << nxt)) == 0) {
			...
			b = overlapCnt(nxt, idx2);
			c = src[nxt].size() - b;
			ans = c + solve(used + (1 << nxt), idx1, nxt);
			if (ans < ret)
				ret = ans;
		}
	}
...
string reconstruct(int used, int idx1, int idx2) {
	...
	for (int nxt = 0;nxt < k; nxt++) {
		if ((used & (1 << nxt)) == 0) {
			...
			ans = c + solve(used + (1 << nxt), idx1, nxt);
			if (ans == solve(used, idx1, idx2)) {
				ret = src[nxt].substr(b)+reconstruct(used + (1 << nxt), idx1, nxt);
